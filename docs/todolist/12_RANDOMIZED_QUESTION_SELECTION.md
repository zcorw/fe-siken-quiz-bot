...
- [x] 梳理当前后 5 道题来源，确认是否已经实现真实错题 / 弱项逻辑，还是仍为高权重 fallback。
- [x] 先写失败测试，覆盖高权重 fallback 候选池足够时不同 seed 选出不同题。
- [x] 先写失败测试，覆盖补强题不能重复使用前 15 道题。
- [x] 如果已有错题候选，先写失败测试覆盖错题优先。
- [x] 实现补强题候选池随机化。
