...
- [x] 匹配前对输入和分类名执行现有 NFKC、日文小写、空白去除归一化。
- [x] 小分类精确匹配优先于大分类精确匹配。
- [x] 一次只接受一个练习范围。
- [x] 如果用户输入明确包含多个范围，返回提示并要求重新输入单个范围。
- [x] 大分类 alias 继续可用，例如 `网络` -> `ネットワーク`。
- [x] 小分类可直接匹配数据库 `category` 名，例如 `通信プロトコル`。
- [x] 若本地无法精确匹配，使用 Fuse.js 给出本地建议。
...
- [x] 更新 AI JSON schema：
  ...
  - 支持返回候选建议。
  - 禁止额外字段。
- [x] AI 匹配结果用于生成候选按钮，不直接创建练习。
- [x] 候选按钮 payload 必须能标识候选类型和名称，例如大分类或小分类。
- [x] 对 AI 返回结果做白名单过滤：
  - 大分类必须存在于分类树顶层。
  - 小分类必须存在于分类树值列表。
- [x] AI 不可用时，继续返回本地建议。
...
- [x] 将大分类转换为小分类列表。
- [x] 查询这些小分类下的全部候选题。
- [x] 按现有选题排序策略选择前 15 道用户指定范围题。
- [x] 保持后 5 道常错 / 弱项 / 高权重逻辑不变。
- [x] `selectionSummaryJson` 中记录命中的大分类和实际使用的小分类。
...
- [x] 成功创建练习时，可继续只发送按钮，不暴露内部分类细节。
- [x] `/start` 文案说明第一版一次只输入一个练习范围。
- [x] `/help` 文案说明第一版一次只输入一个练习范围，并给出单范围示例。
- [x] 无匹配但有建议时，建议文案使用已有大分类 / 小分类名称。
- [x] AI 推荐多个候选时，按配置顺序或相关性排序展示为 Telegram inline buttons。
- [x] 用户点击候选按钮后，再按该候选分类创建练习 token。
- [x] 日文文案保持一致。
