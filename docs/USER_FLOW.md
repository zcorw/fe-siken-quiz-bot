...
9. 若 AI 返回候选，Bot 用 Telegram inline buttons 展示候选，不立即创建测试。
10. 用户点击候选按钮后，系统按该候选分类随机选择 20 题，保存到 session，并创建随机 token。
11. 若仍无可用候选，Bot 用日文返回重试提示，不创建测试。
12. Bot 直接返回 Web 测试链接按钮。
13. 后续打开同一 token 时只读取已保存的 20 题和题号顺序，不重新随机。
...
flowchart TD
    ...
    D --> E{"AI解析後に候補あり"}
    E -- "いいえ" --> F["再入力を促す"]
    E -- "はい" --> Q["候補をTelegramボタンで提示"]
    Q --> G
    C -- "はい" --> G["20問をランダム選定してsessionに固定"]
...
| 用户输入多个范围 | 不生成测试，提示一次只输入一个范围 |
| 指定小分类不足 15 题 | 从同一大分类下兄弟小分类补足指定范围题 |
